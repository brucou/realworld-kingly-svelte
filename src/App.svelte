...
const { fetchGlobalFeed, fetchTags } = apiGatewayFactory(fetch, sessionRepository);
...
const updateProps = {
  ...
  articles: _articles => {
    if (_articles instanceof Error){
      ...
      articles = {data: _articles.articles, fetchStatus: OK, count: _articles.articlesCount};
    }
  }
};
