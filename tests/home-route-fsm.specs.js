...
// TODO
// Route changed
// Auth checked now for all tests
// No need to check in the middle again for auth
// Scenarios home route
// 1. Auth (Y/N)
// 3. Auth (Y/N) x [user actions (like/unlike | filter tag)]^n with n > 1 | [system actions (auth changed, request responses) ]^m
// TODO: think about
// I could have test and generation in the same place?
// Maintain a generator state used to generate the next item in the sequence
// For each (control state) of the generator, run properties
// That would be the best of both worlds probably?
// Gen and Properties could be together put in a textual language?

const NAVIGATED_TO_HOME =   { [ROUTE_CHANGED]: { hash: home } };
const AUTH_CHECKED_NONE =   { [AUTH_CHECKED]: null };
const AUTH_CHECKED_USER =   fixture => ({ [AUTH_CHECKED]: fixture });
